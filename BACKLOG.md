...
### B147: Smart-Paint + STRIPES mode (and STRIPES + fine_top_lines) still not mixing top surface after B146 (GitHub #TBD) — FIXED on `feature/colormix-topsurface`
...
### B144: Mix Send dialog shows model filaments + mislabels mix slots as "support" (GitHub #TBD) — FIXED on `feature/colormix-topsurface`
...
- **Scope**: Kotlin/UI only — no native change. Upload-only (send-to-hold) still ships canonical and is out of scope.
