...
### B147: Smart-Paint + STRIPES mode (and STRIPES + fine_top_lines) still not mixing top surface after B146 — FIXED on `feature/colormix-topsurface`
- **Symptoms** (Kevin, 2026-06-12): after B146, STRIPES mode via Smart Paint still produced no within-layer top-surface split; STRIPES + fine_top_lines also failed; only PROPORTIONAL, DITHER, or STRIPES + ironing_glaze worked.
- **Root cause**: B146's collapse-prevention condition was `top_mix_mode == 1 || top_mix_mode == 2 || ironing_glaze` — STRIPES (`top_mix_mode == 0`) and STRIPES + `fine_top_lines` were not in the list, so those variants still collapsed to a single physical tool per layer. The same `is_mixed()` gate failures described in B146 then applied.
- **Fix**: widen to `mixed_row->enabled` — any enabled painted mix keeps its mix id regardless of mode flags. Engine submodule `colormix-topsurface` @e29830e644. Red→green `PaintedMixTopSurfaceTest#paintedMix_stripesMode_topSurfaceSplits` (new); existing DITHER test green; full regression (unit + instrumented) green on Pixel 8a. APK `u1-slicer-colormix-b147-stripes-painted-f504409.apk` on G:/My Drive/claude. **Lesson**: always test the DEFAULT mode of a sub-feature independently — the simplest/default mode is the one most likely to be omitted from a list.

### B146: Top-surface mix not applied for Smart-Paint (painted) mixes — FIXED on `feature/colormix-topsurface`
...
- **Fix**: new `Print::mix_expanded_used_filament_count(bool)` expands virtual mix ids to component physical tools (via `MixedFilamentManager::expand_virtual_extruder_ids`, `num_physical = filament_diameter.size()`) and counts distinct; used for `used_filaments` at both `normalize_fdm_2` call sites. Engine submodule `colormix-topsurface` @b9b4e44d61. Red→green `StlMixPrimeTowerTest`; regressions green (TopSurfaceMix*×7, SlicingIntegrationTest 54/54 — no spurious tower on plain STL / multi-colour 3MF).

### B144: Mix Send dialog shows model filaments + mislabels mix slots as "support" — FIXED on `feature/colormix-topsurface`
- **Symptoms** (mix-assigned slice): tapping Map & Print opens "Filament mapping" listing the model's file filaments (e.g. red/black) PLUS the mix's physical component slots (E3 pink / E4 blue) tagged "support". The body is already physical-slot space (B142b), so the canonical file-filament view is wrong.
- **Root cause**: the `plateNarrowed` remember in `MainActivity` derived the dialog list via `computePlateFileIndices`, which returned null for a mix (usage on slots beyond the model's canonical size) → fell into `buildWideGcodeMapping`, which synthesised the extra slots as `FilamentSource.SUPPORT_FILAMENT`. The Send path never learned about `sliceMixToolSpace` (only the inline preview + Slice Summary did).
- **Fix**: `buildMixSlotMapping(perExtruderFilamentMm, extruderPresets)` lists active physical slots (one row per slot with filament used), each carrying that slot's preset colour + material, tagged new `FilamentSource.PHYSICAL_SLOT`. Wired as the first branch of `plateNarrowed` gated on `viewModel.sliceMixToolSpace`; `FilamentMappingDialog` gets a `physicalSlotSpace` flag that relabels rows "E<n>" + retitles the header. Picker seeds identity (slot→same slot); `applyPrintTimeRemap` leaves it untouched unless re-mapped; mismatch warning uses per-slot preset materials. Tests: `BuildMixSlotMappingTest` (4), `MixSendDialogWiringTest` (2 structural).
- **Scope**: Kotlin/UI only — no native change. Follow-up review fixed mix-tool-space Upload Only/Save/Share/Jobs handling by persisting G-code tool space and showing physical E-slot upload confirmation for mix bodies.

### F97: ColorMix — within-layer top-surface mixing + per-mix modes — DONE on `feature/colormix-topsurface` (unreleased)
...
- **Verification**: TopSurfaceMixModesTest 5/5 + TopSurfaceMixWipeTowerTest 2/2 + MixSlot*×7 + SemmSlicingTest + SlicingIntegrationTest (85 instrumented total) + full JVM suite, all green on Pixel 8a. Engine submodule branch `colormix-topsurface` @32eac5e913. APK staged: `G:\My Drive\claude\u1-slicer-colormix-modes-beta-f9caea5.apk`. Awaiting Kevin's print verdict and public release decision.
...
### B143: G-code preview layer label tops out at N-1/N — final layer looks unreachable — FIXED on `feature/colormix-topsurface`
...
- **Fix**: `computeDisplayLayer()` in `ui/GcodeViewer3DScreen.kt` scales the 1-based position (extremes map to exactly 1 and N); 6 unit tests in `GcodeLayerRangeTest`.
...
- **B142b follow-on (same day)**: the sliced G-code preview + per-extruder summary showed red/black + two grey tools — mix slices emit **physical-slot tools** (`mixPhysicalBase` sizes the filament arrays to TARGET_SLOTS; mixes alternate physical component tools) but the post-slice palettes stayed canonical-file-space. Fixed: `SlicerViewModel.sliceMixToolSpace` (set from `anyMixAssigned` at slice time) + `slotPaletteWithMixBlends` (presets E1..E4 + mix blends); inline + full-screen G-code viewers drop the canonical mapping and use the slot palette; per-extruder summary rows show slot colours/materials with E1..E4 labels. Guard in `MixSwatchPaletteSourceTest`.

### B141: Assigning a part to a mix on a 3MF doesn't update the Prepare preview (GitHub #178) — FIXED on `feature/filament-library`, RELEASED v3.2.0-beta (released 2026-06-10)
...
- **Fix**: prefer the live set whenever non-empty (it only populates after an assignment and resets on load/plate switch); file-declared set remains the pre-assignment fallback. `buildCanonicalNonMmuPalette` mix-id colour math unchanged. Guard: `MeshPaletteLiveSourceTest`.
- **Reported by**: Kevin, 2026-06-10, during the F96 sanity test (follow-on from B140). Pre-existing M3/M4 mix bug; fix rides with the F96 branch.

### B140: Object-selector mix swatches use model filament colours instead of printer slot colours (GitHub #177) — FIXED on `feature/filament-library`, RELEASED v3.2.0-beta (released 2026-06-10)
...
- **Root cause**: `BambuSanitizer.process()` takes the streaming sanitize path when the main `3dmodel.model` > `MAX_IN_MEMORY_MAIN_MODEL_BYTES` (64MB). That path skips the in-memory compound-restructuring block, so `attachCompoundComponentIds` never runs, `isCompound=false`, and the fallback `buildSlic3rModelConfig` writes a `Slic3r_PE_model.config` with NO `subtype` (and firstid/lastid ranges meaningless for component meshes). `embedProfile(processed,…)` ([SlicerViewModel.kt:3927](app/src/main/java/com/u1/slicer/SlicerViewModel.kt#L3927)) embeds that file; `convertToModelSettings` also only emits `extruder`. The native BBS importer derives volume type solely from `<part subtype>` ([bbs_3mf.cpp:4794](app/src/main/cpp/orcaslicer/src/libslic3r/Format/bbs_3mf.cpp#L4794)) → no subtype → `MODEL_PART` → solid.
- **Fix**: In `process()`'s config decision, when the streaming path was used (`usedStreamingMainModel`) but the source `model_settings.config` already has Bambu's compound `<part>` format (part ids = component objectids, carrying subtype + per-part extruder), preserve that original config (extruder `0`→`1` normalised, B77) instead of regenerating a subtype-less one. Mirrors the existing `deferredRestructuring` preserve branch; gated so the in-memory path is provably unchanged.
- **Sibling fixes folded in (same streaming-metadata-loss class)**:
  - Per-volume **extruder** assignments were also lost for >64MB *multi-color* inlined compound files — same fix restores them (original config carries per-part extruder).
  - `copyMainModelEntry` / `copyZipEntry` (streaming main-model + large-component paths) **deleted** `slic3rpe:mmu_segmentation` instead of renaming it → >64MB PrusaSlicer SEMM files lost all paint and sliced single-colour. Changed to rename → `paint_color=` for parity with `convertMmuSegmentation` / embed's `streamCleanEntry`.
- **Tests**: `NegativeVolumePreservationTest#largeCompoundModel_negativeVolumesSurviveProcessAndEmbed` (RED before fix = 0 negative_part in embedded config; GREEN after = 2). Regression: 75 Bambu-pipeline instrumented tests + full 1453 JVM unit suite pass. The paint-strip sibling fixes are by parity reasoning (no >64MB SEMM fixture available to red-green).

### B138: ProfileEmbedder.convertToModelSettings drops per-volume subtype for PrusaSlicer (non-Bambu) modifier 3MFs (GitHub #171) — OPEN 2026-06-04
- **Symptom (theoretical, not yet repro'd on-device)**: A genuine PrusaSlicer-format 3MF (ships `Metadata/Slic3r_PE_model.config`, not Bambu `model_settings.config`) that uses negative/modifier volumes would have them flattened to normal parts — `convertToModelSettings` ([ProfileEmbedder.kt:81-122](app/src/main/java/com/u1/slicer/bambu/ProfileEmbedder.kt#L81)) only emits `extruder`, never `volume_type`/`subtype`.
- **Why separate from B137**: Different file format and code path. B137's fix means Bambu modifier files no longer reach `convertToModelSettings`, but PrusaSlicer-authored files still do.
- **Scope/priority**: Low — U1 users predominantly use Bambu/Orca-format files. Needs a PrusaSlicer modifier fixture to red-green.
- **Fix sketch**: Parse and re-emit `volume_type`/modifier metadata in `convertToModelSettings` (map PrusaSlicer's volume-type metadata → `subtype`), or detect modifier volumes and preserve the source config like B137 does.
...
### B124: Button-for-S-trousers cannot be moved on bed — single multi-volume 3MF triggers multi-object renderer mode (GitHub #143) — FIXED v2.2.14, REGRESSION REPAIRED v4.0.3 (released 2026-08-29)
...
- **v4.0.3 regression repair**: an F100 import-exposure call promoted every multi-volume 3MF, including this single object with many volumes, back into multi-object mode. Removing that call restores the B124 invariant. The three exact B124 instrumented tests and the complete 449-test instrumented sweep passed after the repair.
...
### F98: Global ironing override in slicing settings — DONE v4.0.3 (released 2026-08-29)
- **Request**: expose OrcaSlicer's ordinary ironing setting in the phone slicer's settings, independently of the existing ColorMix-only “ironing glaze”.
- **Implementation**: Layer & Infill now offers Off, Top surfaces, Topmost surface, and All solid layers. Like every other override it has Use File / Orca default / Override semantics: Use File preserves a 3MF's `ironing_type`; the selected value is embedded for 3MF/Bambu jobs and travels through `SliceConfig`/JNI for raw STLs too.
- **Scope**: advanced ironing pattern, flow, spacing, inset, speed, and angle remain deferred; this first pass exposes the meaningful on/off/surface-mode choice without pretending to offer a full desktop expert panel.
- **Verification**: red→green persistence, profile embedding, native-sentinel, and Bambu explicit-precedence tests; NDK 26 Release `.so` rebuilt/stripped (Clang 17.0.2; 21.7 MB; 60 JNI symbols = 60 Kotlin declarations).

### F99: Remap imported 3MF file colours to ColorMix recipes — DONE v4.0.3 (released 2026-08-29)
- **Delivered**: Prepare now has a dedicated file-colour remap sheet. Every canonical 3MF colour can target U1 E1–E4 or a saved ColorMix recipe; the physical-only Map & Print dialog remains unchanged.
- **Pipeline/safety**: remaps are persisted by canonical `fileIndex` and stable mix ID. Immediately before re-embed/slice, a disposable 3MF rewriter lowers every paint-tree leaf and component `extruder` metadata into one physical/mix tool space, preserving the raw import and unrelated ZIP entries. Mix remaps stamp the four-physical-tool full-spectrum recipe, force prime-tower planning, and suppress the old canonical G-code remap so a staged physical T command is never remapped twice. Deleted recipes or components outside E1–E4 block slicing with an actionable error.
- **Preview/session**: source meshes retain their canonical topology but display the chosen physical-slot/mix blend colours; G-code uses the physical palette after slicing. Project mixes and file-colour destinations round-trip with session restore before structural replay.
- **v4.0.4 reliability repair**: once the disposable pre-slice 3MF has lowered a source colour into a virtual ColorMix tool, that tool id is preserved through the native reload instead of being mistaken for a later canonical source colour. This fixes the five-canonical-colour collision where source E3 is mapped to a mix and both physical recipe tools must reach G-code.
- **Verification**: JVM tests cover canonical identity (including duplicate source colours), stable/missing/invalid mix resolution, physical tool-space decision, split/extended paint-state re-encoding, component-metadata rewriting, ZIP preservation, and session JSON round-trip. Device coverage includes the standard file-colour mix path and the five-colour E3 collision case, asserting both physical mix tools in G-code.

### F100: Delete individual objects and expose already-separated 3MF objects — DONE v4.0.3 (released 2026-08-29)
- **Delivered**: multi-object 3MF plates now retain their authored object positions and show an **Objects** picker. Users can select any imported object and use **Delete this object**; the final remaining object is protected.
- **Safety/state**: native deletion runs under the preview mutex and keeps the preview-extruder cache, transform baselines, positions, bounding boxes, per-volume extruders, and selection aligned. Structural edits now persist as an ordered model-operation log, so interleaved split/copy/delete actions replay correctly on slice and session restore.
- **Verification**: focused JVM coverage for operation-log persistence and delete remapping; native NDK 26 Release library rebuilt/stripped (Clang 17.0.2; 21.73 MB; 61 JNI symbols = 61 Kotlin declarations); both native delete cases pass on the approved NF21F1 test phone. Gradle's UTP listener could not start locally, so the identical compiled instrumentation APK was installed and run directly via `am instrument`.

### F101: Variable/adaptive layer height controls — DONE v4.0.3 (released 2026-08-29)
- **Delivered**: Prepare offers project-scoped Use file, Off, and Adaptive controls, with Detail/Balanced/Fast presets and a live minimum/maximum layer-height and profile-point count readout. The desktop-style painted curve editor remains deferred.
- **Implementation/safety**: Orca 2.2.4's `adaptive_layer_height` config key is inert in this vendored build, so the app correctly generates each native `ModelObject.layer_height_profile` via Orca's adaptive/smoothing APIs rather than serialising a no-op profile key. Profiles regenerate after re-embed, structural edits, and pose replay immediately before slicing; session state persists the recipe rather than fragile raw points. Adaptive slicing is blocked for prime-tower, multi-tool, and ColorMix jobs because variable layers cannot safely align the tower.
- **Verification**: session JSON coverage includes all adaptive state; NDK 26 Release library rebuilt and stripped (Clang 17.0.2, 21.74 MB, 64 JNI symbols matching Kotlin). The approved NF21F1 test phone passes the native create/report/clear smoke test. Complete JVM suite passes.

### B148: Share action can appear to do nothing when a job's G-code is missing — INVESTIGATE (new intake, 2026-08-26)
- **Finding**: current-slice share builds a FileProvider `ACTION_SEND` chooser correctly. The historical-job path returns silently when its file is missing, which can look like a dead Share button.
- **Plan**: reproduce on-device; if confirmed, surface the same actionable “G-code file not found” message used by the job viewer. Do not change the working share path without a reproduction.

### B149: Bespok3d Desktop-painted 3MF is rejected or loses its painting — INVESTIGATE (new intake, 2026-08-27)
- **Reporter**: Jon (Discord). In [Bespok3d Desktop](https://github.com/Bespok3d/Bespok3d-desktop), paint the model, choose to use that painting, then import/slice it in U1 Slicer; the app reports failure (“get rekt” in the report) rather than producing the painted result.
- **Fixture**: `C:\Users\kevin\Downloads\鍵帽組合版.3mf` (7,120,807 bytes). It is a Bambu Studio 3MF with one 402,392-face object assigned to E3. Its supplied XML has no standard `paint_color`, `mmu_segmentation`, or `slic3rpe:mmu_segmentation` attributes, so establish whether Bespok3d's chosen painting is exported into this 3MF, held in a sidecar/project representation, or encoded in an unsupported form before attributing the failure to the import path.
- **Repro/evidence to collect**: Bespok3d Desktop version, exact “use this painting” action, resulting file path(s), complete U1 Slicer error text/log, and an exported 3MF after painting. Compare ZIP entries and XML against a known Bambu/Orca painted fixture.
- **Acceptance**: either import and preserve the documented Bespok3d painting representation through Prepare → Slice → G-code, or show a precise unsupported-painting/export message with an actionable export path. No silent fallback to the base E3 object.
- **Out of scope for intake**: implementing a decoder or changing the 3MF pipeline before the exported representation is confirmed.

### F102: Choose the physical print-camera stream when Bespok3d Desktop provides a virtual display camera — DONE v4.0.3 (released 2026-08-29)
- **Request**: Jon has replaced PAXX Extended with Bespok3d Desktop. Bespok3d exposes the printer display as a camera too; U1 Slicer currently selects that virtual/display feed, not the physical print camera. Let the user choose the stream so they can monitor the actual print.
- **Context**: [Bespok3d Desktop](https://github.com/Bespok3d/Bespok3d-desktop) is a beta, printer-agnostic desktop/plugin system for stock Klipper printers. Treat its camera endpoints and labels as external, potentially evolving capabilities.
- **Scope**: discover and list every compatible camera stream for the selected printer; preserve the user's stream choice per printer; use it for the Printer-tab preview/fullscreen viewer; provide a clear unavailable/error state. Keep the existing automatic choice as a sensible default when only one stream exists.
- **Acceptance**: with both Bespok3d's display camera and the physical print camera available, the Printer tab exposes distinguishable choices and renders the selected physical feed across app restart. Verify no regression for ordinary single-camera printers.
- **Out of scope**: installing/configuring Bespok3d, changing its daemon, or inferring an endpoint from a hard-coded camera name.
- **Delivered**: Moonraker camera discovery now keeps each compatible camera as a named source with its own snapshot fallbacks. The Printer tab offers a picker when more than one source is available, and persists the selected Moonraker camera UID per printer. The existing single-camera fallback remains unchanged, and Bambu camera handling is unaffected.
- **Validation**: Moonraker parsing and selection persistence are covered by JVM tests. The full 1,975-test JVM suite, all 449 instrumented tests, and 34/34 manual U1/H2D E2E scenarios passed for v4.0.3.
...
- **Sync matching**: `FilamentLibraryMatcher` identifies the catalogue filament from RFID-derived vendor/type/subtype/colour (gate on raw CIE76 ΔE ≤ 10 FIRST, then subtype-token bonus ranks survivors; vendor brand containment both ways; never guesses across brands — Snapmaker stock tags intentionally fall through to raw sync). Matched sync rows show "<name> (matched)" and apply catalogue colour/material; recents recorded on apply.
...
- **QA fixes (on-device, post-implementation):**
  - **Object-selector mix rows clipped on models with many filaments — FIXED, commit 4172ff8**: `PartsPanel.FilamentChooserDialog` clipped its mix rows and the "+ Add mix" button below the dialog fold on models with many canonical filaments (e.g. Button-for-S-trousers = 15 filaments). The extra rows looked missing and the user couldn't add a mix from the per-object dialog. Fixed with `verticalScroll` on the dialog content `Column`; `FilamentChooserDialogScrollTest` guards it. Device-verified.
  - **"Same-layer dots" mix mode removed — commit 02bff22**: The toggle (native `SameLayerPointillisme`) was a no-op — verified in Snapmaker Orca C++ that pointillism is `#if 0`-disabled, marked "Deprecated … disabled and will be removed" (`PrintApply.cpp`), auto-converted to `LayerCycle` before slicing, and absent from upstream Prusa/Bambu. Removed the toggle from the UI (mixes always use Layer alternation); the enum value is kept for JSON back-compat; `MixSelectorAugmentationTest` guards that the toggle stays gone.
- **Residual limitations** (OPEN follow-ups, not blocking release):
...
### Pick-a-colour: choose a target colour → suggest the closest filament mix — DONE v3.1.0-beta (released 2026-06-10)
The chosen full-spectrum follow-up after M4 — the "colour prediction" direction from the M5 brainstorm (built instead of an M5-A auto-palette first). Lets the user pick a colour and have the app suggest which loaded filaments + ratio get closest, built on accurate forward colour prediction. Pure Kotlin/UI — **no native change, no `.so` rebuild**.
- **Forward predictor** ([`FilamentMixPredictor.kt`](app/src/main/java/com/u1/slicer/aipaint/FilamentMixPredictor.kt)) — faithful Kotlin port of `prusa3d/prusa-fdm-mixer` (MIT) @ `75fcd72`: Yule-Nielsen base (n=3) + CIELAB lightness/chroma/cyan-band-hue corrections; native N-way (no pairwise folding). Pinned to the repo's 7 reference vectors at ΔE<1. Now drives the displayed mix swatch + 3D preview colour at the 3 display sites (`SlicerViewModel`, `NavGraph`, `AiPaintResultScreen`), replacing the crude `ColourMatch.naiveBlendHexMulti` sRGB average.
- **Reverse matcher** ([`MixColourMatcher.kt`](app/src/main/java/com/u1/slicer/aipaint/MixColourMatcher.kt)) — `bestMix(target, loaded, count)` brute-forces filament subsets × a coarse weight grid + fine local refine, scored by `ColourMatch.deltaE76`; `closestSingleFilament` for the "a plain filament is closer" note. ~8 ms/query for 4 filaments.
- **UI** — a "🎯 Match a colour" button + 2/3/4 count selector (default 2, capped to loaded) in `CreateMixSlotDialog` (under the existing BETA pill); reuses the F64 HSV picker (`FilamentColorEditDialog`) for the target; auto-fills the existing weight-bar editor with the suggestion + a Good/OK/weak ΔE badge.
- **v1 = "suggested/preview"** — absolute printed colour is not guaranteed; **colour calibration** (printed-swatch capture → per-filament/per-printer correction) and **sample-from-photo/model** target input are deferred (see spec §9). A standalone "Pick a colour" entry is a possible later add.
- **Tests**: `FilamentMixPredictorTest` (5), `MixColourMatcherTest` (8), `MatchAColourWiringTest` (2), instrumented `MatchAColourE2ETest` (1, device-verified: matched mix slices with exactly the suggested tools). Full 1603 JVM unit sweep green.
- **Spec/plan**: [`docs/superpowers/specs/2026-06-09-pick-a-colour-mix-matching-design.md`](docs/superpowers/specs/2026-06-09-pick-a-colour-mix-matching-design.md), [`docs/superpowers/plans/2026-06-09-pick-a-colour-mix-matching.md`](docs/superpowers/plans/2026-06-09-pick-a-colour-mix-matching.md).
...
- **v4.0.4 (released 2026-08-29):** Fixed F99 file-colour to ColorMix remapping after the staged 3MF reload: a virtual mix tool can no longer collide with a fifth canonical file colour and collapse to one physical tool. The manual E2E runner now waits for the actual app plate selector rather than raw 3MF metadata, preventing a non-Bambu single-plate model from being selected while it is still loading; the model-load boundary also clears previous plate state immediately. Validation: 1,976 JVM tests, 451 instrumented tests, and 34/34 U1/H2D manual E2E scenarios passed. B149 remains open because its Bespok3d paint representation has not yet been confirmed.

- **v4.0.3 (released 2026-08-29):** Added a normal ironing override, file-colour remapping to U1 slots or ColorMix recipes, adaptive layer-height controls, individual-object selection/deletion for already-separated 3MFs, and persisted Moonraker camera selection. Repaired the B124 single multi-volume 3MF regression found during the release sweep. Validation: 1,975 JVM tests, 449 instrumented tests, and 34/34 manual U1/H2D E2E scenarios passed. B149 remains open because the supplied Bespok3d-painted 3MF has no confirmed standard paint representation.
...
**v3.3.4 (released 2026-06-16)** - Prepare preview regression follow-up for GitHub #181: the Kotlin preview-budget fix now actually reaches the native preview path, and the native sparse fallback is restricted to huge flat painted files instead of all >2M-triangle painted models. Chubby Darth Vader now renders correctly on Prepare and passed focused JVM + native regression tests plus Smoke-7 E2E on Pixel 8a. Ghostface remains an explicit follow-up under F99 adaptive preview quality rather than being treated as fully solved.
...
**v3.3.0 (released 2026-06-14)** — F97 ColorMix top-surface mixing + per-mix modes (STRIPES / PROPORTIONAL / DITHER, fine_top_lines, ironing_glaze) plus the B143/B144/B145/B146/B147 cluster: final-layer preview label fix, mix send-dialog slot-space wiring, single-object STL+mix prime tower gating, painted mix top-surface split, and STRIPES painted mix collapse fix. 1699 unit + 433 instrumented + 27/27 E2E all green on Pixel 8a.

**v3.2.0-beta (released 2026-06-10)** — F96 OpenPrintTag filament library (12,804 filaments / 112 brands bundled, MIT): Library tab in physical-slot dialogs with search/material chips/favourites/recents, pick = colour+material, opt-in profile import with preview + name-dedupe, conservative RFID sync matching (CIE76 ΔE ≤ 10 gate, matched rows show catalogue names), Settings ▸ About attribution, `tools/openprinttag-convert` snapshot tooling (GitHub #176). Plus the B140/B141/B142(+b) pre-existing M3/M4 mix display cluster found in Kevin's sanity test (GitHub #177/#178/#179): mix swatches/blends from printer slot presets, live-set recolor palette, slot-space post-slice G-code palette + per-extruder summary (E1–E4 labels), mix-aware filament chip; CREATE_MIX/ASSIGN_FILAMENT debug broadcasts for deterministic mix E2E. 1670 unit + 423 instrumented all green; no native change.
See git log for full history. Most recent fixes:
- **F97 / B143-B147**: v3.3.0 — ColorMix top-surface mixing + per-mix modes (STRIPES / PROPORTIONAL / DITHER, fine_top_lines, ironing_glaze) shipped with the release cluster: final-layer preview label fix, mix send-dialog slot-space wiring, single-object STL+mix prime tower gating, painted mix top-surface split, and STRIPES painted mix collapse fix. `TopSurfaceMixModesTest`, `TopSurfaceMixWipeTowerTest`, `StlMixPrimeTowerTest`, `PaintedMixTopSurfaceTest`, and the adjacent smoke/regression sweep all green on Pixel 8a. RELEASED v3.3.0 (2026-06-14). Issues none filed.
- **B137**: v2.10.15 — negative/modifier volumes were sliced as solid objects on large (>64MB main-model) single-file compound 3MFs (articulated print-in-place models with joint-clearance cutters fused solid). The >64MB streaming sanitize path in `BambuSanitizer.process()` skipped compound restructuring → `buildSlic3rModelConfig` dropped per-part `subtype`. Fix preserves the source compound `model_settings.config` on that path (gated on `usedStreamingMainModel`). Same streaming path also fixed: per-volume extruder loss on >64MB multi-colour inlined files, and `copyMainModelEntry`/`copyZipEntry` deleting (now renaming → `paint_color=`) PrusaSlicer SEMM paint data on >64MB files. New `NegativeVolumePreservationTest`. Validated: full unit (1453) + full instrumented (402/403, 1 B130 flake passed isolated) + full 16-file E2E batch. RELEASED v2.10.15 (2026-06-05). Issue #170 closed. (B138 #171 PrusaSlicer-format modifier gap + F95 #172 translucent-modifier preview filed, both open.)
