...
- **Reproduction**: `DC15BuzzRepro.dc15_buzzPlate9_diagnosticDump` and `B95Plate9PaintStateTest.plate9_slicedGcodeHasTwoTools` (@Ignore'd — the latter fails today and unignores once the real fix lands). Both confirm `T0=3 T1=0 T2=0 T3=0`.
- **v1.6.11 investigation** — tried bumping `computeEmbedTargetCount` to `max(usedExtruderIndices)` so the embedded `filament_colour` has 10 entries and the native virtual-extruder padding path (B48) triggers. Confirmed via `B95EmbedDumpTest` that the embed reaches size 10 with the correct Bambu filaments (including peach #FFD6C1 and white #FFFFFF) — but the slicer STILL emits only T_<default>. Paint segmentation isn't processing the paint state even when `filament_colour.size()+1` is well above the state index reported by the Kotlin parser. The attempt also regressed plate 8's Preview palette because `semmColorPermutation` expanded to 10 entries confused `normalizeGcodePreviewColors`'s B92 loop. Reverted.
- **What the investigation revealed**: the `paint_color` attribute values on plate 9 are bit-packed (`"8C"`, `"3C"`, etc. — not a simple state-char sequence). OrcaSlicer's `TriangleSelector::deserialize` decodes these into triangle states, but the Kotlin `paintCharToState` heuristic that drives `usedExtruderIndices` / `detectedColors` uses the first char as a state index. The two disagree for plate 9, and that disagreement is why `max(usedExtruderIndices)` doesn't actually tell the native slicer anything useful about which states its segmentation will see. The correct fix likely needs either a faithful bit-packed decoder (to map source filament indices to compact ones in the sanitizer) or a native patch inside `multi_material_segmentation_by_painting()`.
- **Not affected**: Plate 1 (4-colour object-extruder) emits all 4 tools; plate 8 (object 10 + paint state 3, lower-indexed) emits T0+T3 correctly.
...
### B92: Prepare ↔ Preview colour swap on per-object + paint-state SEMM plates (GitHub #96) — FIXED v1.6.10
