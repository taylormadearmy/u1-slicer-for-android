...
### B125: H2C shoe support filament row missing — support_filament not emitted when supports=USE_FILE + hasSourceConfig=true (GitHub #144) — FIXED v2.2.14
...
- **Root cause**: `buildProfileOverridesImpl` in `SlicerViewModel.kt` has an early-exit gate: `if (ov.supports.mode != OverrideMode.USE_FILE || !hasSourceConfig)`. For Bambu 3MF files (`hasSourceConfig=true`) where the user has not explicitly overridden the supports toggle (remains `USE_FILE`), the ENTIRE block — including `support_filament` and `support_interface_filament` — is skipped. The gate was added by B10 to preserve the file's embedded `enable_support` value. But `support_filament`/`support_interface_filament` are independent: the user may want to change the support extruder without touching support enable/angle. The embedded file value `support_filament=2` is used instead of the user's E4 choice. This causes all support to run on E2 (embedded) rather than the user-selected extruder, and `computePlateFileIndices` / `buildWideGcodeMapping` never see an active index beyond canonical size → 1 row in dialog.
- **Fix**: Move `support_filament` / `support_interface_filament` emission outside the `ov.supports` gate. Emit whenever `ov.supportFilament.mode == OverrideMode.OVERRIDE` (explicit user choice) OR when `supportBlockActive` (non-Bambu path unchanged). Add `supportBlockActive` local to capture the original gate condition for the fallback path.
- **Tests**: 3 new unit tests in `SlicingOverridesTest.kt` (`B125 support_filament emitted for Bambu file when supportFilament is OVERRIDE`, `B125 support_filament omitted for Bambu file when supportFilament is USE_FILE`, `B125 support_filament emitted for STL file even when supports is USE_FILE`).
...
- **Tests**: 1 structural source-grep test in `InlineModelPreviewRotationKeysTest.kt` (`inlineModelPreview_perObjectSizesGatedOnHasMultipleDistinctObjects`) that verifies the gate is wired in MainActivity. Multi-object behaviour (F77, multi-file STL) is unchanged since `hasMultipleDistinctObjects` is still set `true` via `doAddFile`.
