...
### B131: Ghostface Pokemon card 3MF not visible on Prepare tab — slice/preview work, model can't be moved to clear purge tower (GitHub #161) — INVESTIGATED, FIX DEFERRED
...
- **Reported by**: Jon (Discord), v2.10.1, 2026-05-31.
- **2026-06-01 empirical findings (`B131B132B133DiagnosticTest`, Pixel 8a)**:
  - File loads as **1 native object** (not 483 components as the initial hypothesis predicted). `nativeGetObjectCount=1`, `boxes=3floats (1 object)`, `hasMultipleDistinctObjects=false`. So the original multi-object hypothesis was wrong — `multiObjectMode` correctly stays false.
  - **Bug 1 — decimation cap is bypassed**: `getPreparePreviewMesh(MAX_DECIMATED_TRIANGLES=100_000)` returns **3,727,790** triangles. The native QEM/stride decimation path has a code branch that ignores the cap for this kind of file (Hueforge-style flat painted card — 147×223×3mm with paint state per Z layer). Decimation wall time is ~1.9s, so timing isn't the user-facing symptom by itself; it's the 3.7M-tri mesh upload to the GL buffer.
  - **Bug 2 — mesh world-coord vs `loadTimeInstanceOffsets` mismatch**: mesh AABB starts at world `(47.5, 18.8)` (i.e. mesh vertices are already in world coords), but `getInstanceOffsets()` returns `(116.3, 137.2)` — a non-matching value the renderer treats as the mesh's bottom-left target. `drawModelAt(mesh, 116.3, 137.2)` shifts the mesh from world (47.5..194.9, 18.8..242.5) to world (116.3..263.7, 137.2..360.9). Y range ends at 360mm — **~91mm past the 270mm bed edge**. The user perceives "model never shows" because it's drawn way off the bed reference frame.
  - `nativeGetObjectWorldAABBMins()` returns `(47.5, 18.8)` — matches the mesh AABB. So the engine knows the correct world position; `getInstanceOffsets()` returns something else (likely the BBS object-origin or instance-translation component, which doesn't match mesh world-min for files with non-identity instance transforms).
- **Why the fix is deferred**: the cleanest fix would change either (a) the renderer's interpretation of `instancePositions` (could break Benchy and other STL paths where mesh is in local coords centred around 0), or (b) the `loadTimeInstanceOffsets` derivation (use `nativeGetObjectWorldAABBMins()` for single-object loads when mesh is in world coords — risk: wrong for STL loads where world-min and instance-offset both equal zero). Both fixes need careful tracing of all single-object load paths (STL, single-plate Bambu, plate-selected Bambu, post-rotation reload) to avoid regressions. Separately, the decimation-cap bypass is a native (C++) bug requiring a `.so` rebuild.
- **Next steps when picked up**: (1) Bisect `getPreparePreviewMesh` in `sapil_model.cpp` to find why the cap is ignored for this file (likely the "MMU interleaved" or "flat" branch around line 536+); (2) Trace `_loadTimeInstanceOffsets` consumers in `MainActivity` to determine whether using `nativeGetObjectWorldAABBMins()` is safe across all single-object load paths; (3) Add `B131B132B133DiagnosticTest` to the regression sweep so the fix can be verified.
...
### F94: Immediate "Preparing G-code" indicator during large-file send — bridge the silent gap before "Uploading G-code" appears (GitHub #166) — OPEN
...
### F92: Auto-arrange — match auto-orient parity (Arrange on Bed) (GitHub #162) — OPEN
