...
### B125: H2C shoe support filament row missing — support_filament not emitted when supports=USE_FILE + hasSourceConfig=true (GitHub #144) — FIXED v2.2.14 (filament keys) / v2.2.15 (all sub-settings)
...
- **Fix (v2.2.15)**: Extend to all support sub-settings. Extract `supportBlockActive` before the gate. Keep `enable_support` gated as before (B10). Each sub-setting now emits when `supportBlockActive || ov.<field>.mode == OVERRIDE`, so a stale OVERRIDE on e.g. `supportType` or `supportSpeed` is not silently dropped when the user reverts the supports toggle to USE_FILE.
- **Tests**: 3 unit tests for filament keys (v2.2.14) + 3 new unit tests for sibling settings in `SlicingOverridesTest.kt` (v2.2.15): `B125 siblings support_type emitted for Bambu file when OVERRIDE`, `B125 siblings support_type omitted for Bambu file when USE_FILE`, `B125 siblings supportSpeed emitted for Bambu file when OVERRIDE`.
