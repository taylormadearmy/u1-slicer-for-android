...
./gradlew connectedDebugAndroidTest                # 213 instrumented tests — uses Orchestrator
```
...
### Instrumented tests (`app/src/androidTest/`) - 206 tests across 19 classes
...
- `native/NativeLibraryCorrectnessTest.kt` (12) — JNI correctness checks + Phase 1 sub-plan #1 accessors (`nativeGetObjectCount`, `nativeGetVolumeCount`, `nativeGetObjectModelId`, `nativeGetVolumeScalars`, `nativeGetPaintStateCounts` for both mmu and supports kinds) + sub-plan #5 accessor (`nativeGetProjectConfig` populated JSON + null on no-model)
