...
class MixedFilamentRowTest {
    ...
    fun `autoLabel formats components and percent`() {
        ...
        assertEquals("E1+E3", MixedFilamentRow.autoLabel(listOf(row.componentA, row.componentB)))
    }

    @Test
    fun `autoLabel handles odd percentages`() {
        assertEquals("E2+E4",
            MixedFilamentRow.autoLabel(listOf(2, 4)))
    }

    @Test
    fun `equality treats id as identity`() {
        val a = MixedFilamentRow.fromLegacy(1L, 1, 2, 50, MixedFilamentRow.MixDistributionMode.LAYER_CYCLE, "x", false)
